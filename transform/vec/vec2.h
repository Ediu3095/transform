...
// template <typename T> constexpr Vec<L, T>::Vec(T head, Vec<L - 1, T> tail) noexcept : head_(head), tail_(tail) {}

// --- Conversion constructors ---
template <typename T> template          <typename A, typename... B> constexpr Vec<L, T>::Vec(       A  head, B... tail) noexcept : head_(head       ), tail_(             tail...) {}
template <typename T> template          <typename A, typename... B> constexpr Vec<L, T>::Vec(Vec<1, A> head, B... tail) noexcept : head_(head.head()), tail_(             tail...) {}
template <typename T> template <usize M, typename A, typename... B> constexpr Vec<L, T>::Vec(Vec<M, A> head, B... tail) noexcept : head_(head.head()), tail_(head.tail(), tail...) {}
