...
TEST(VecTest, Vec2) {
  ...
  EXPECT_EQ(Vec2<i32>(vecn).x(), 1);
  EXPECT_EQ(Vec2<i32>(vecn).y(), 2);

  EXPECT_FLOAT_EQ(Vec2<f32>(vecn).x(), 1.0F);
  EXPECT_FLOAT_EQ(Vec2<f32>(vecn).y(), 2.0F);
  ...
  EXPECT_EQ(+vecn, Vec2<i32>(+vecn.x(), +vecn.y()));
  EXPECT_EQ(-vecn, Vec2<i32>(-vecn.x(), -vecn.y()));

  EXPECT_EQ(vecn + 1, Vec2<i32>(vecn.x() + 1, vecn.y() + 1));
  EXPECT_EQ(vecn - 1, Vec2<i32>(vecn.x() - 1, vecn.y() - 1));
  EXPECT_EQ(vecn * 1, Vec2<i32>(vecn.x() * 1, vecn.y() * 1));
  EXPECT_EQ(vecn / 1, Vec2<i32>(vecn.x() / 1, vecn.y() / 1));
  EXPECT_EQ(vecn % 1, Vec2<i32>(vecn.x() % 1, vecn.y() % 1));

  EXPECT_EQ(1 + vecn, Vec2<i32>(1 + vecn.x(), 1 + vecn.y()));
  EXPECT_EQ(1 - vecn, Vec2<i32>(1 - vecn.x(), 1 - vecn.y()));
  EXPECT_EQ(1 * vecn, Vec2<i32>(1 * vecn.x(), 1 * vecn.y()));
  EXPECT_EQ(1 / vecn, Vec2<i32>(1 / vecn.x(), 1 / vecn.y()));
  EXPECT_EQ(1 % vecn, Vec2<i32>(1 % vecn.x(), 1 % vecn.y()));
